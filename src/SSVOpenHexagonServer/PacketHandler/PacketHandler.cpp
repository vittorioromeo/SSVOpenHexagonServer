...
std::string getMD5Hash(const std::string& mString) { ssvu::MD5 key{mString}; return key.GetHash(); }
...
bool handlePacket0x00(sf::Packet &packet, MySQLSession &session, sf::TcpSocket &socket, bool verbose)
{
	...
	if(packet >> level >> difficulty >> username >> score >> md5hash)
	{
		...
		{
			if(verbose) std::cout << "Hash comparison failed." << std::endl;
			sf::Packet response = buildPacket0x10(1);
			socket.send(response);
			return false;
		}
		else if(verbose) std::cout << "Hash comparison succeeded! Continuing.." << std::endl;
	...
	{
		if(verbose) std::cout << "Error, packet 0x00 did not extract successfully." << std::endl;
...
R"(SELECT CONCAT(
	...
	{
		if(verbose) std::cout << "Error: packet 0x01 did not extract successfully." << std::endl;
