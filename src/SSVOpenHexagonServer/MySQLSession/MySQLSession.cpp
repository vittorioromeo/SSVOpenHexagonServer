...
MySQLSession::MySQLError MySQLSession::getError()
{
	...
	if(verbose) std::cout << "MySQL Error " << std::to_string(returnValue.errorno) << " : " << returnValue.errorstr << std::endl;
	return(returnValue);
}
...
std::string MySQLSession::runQuery(std::string queryString, bool get)
{
	...
	result = mysql_store_result(connector); // TODO: initialize instead of assigning?
	...
	while((row = mysql_fetch_row(result)))
	{
		...
		{
			if(verbose) std::cout << (row[i] ? row[i] : "NULL");
...
void MySQLSession::closeSQL() { if(!initialized) return; mysql_close(connector); }

MySQLSession::MySQLSession(bool verbose_)
{
	verbose = verbose_;
	connector = mysql_init(NULL);
	if(connector == NULL) getError();
}
